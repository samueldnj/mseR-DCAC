...
.initPop <- function( obj )
{
  ...
  # Simulate two qs. qSurvey applies for
                     ...
                     tolSD = 0.1, tolMult = 1 )
{
  ...
  while ( lSim < length )
  {
    # Compute the max length of a new piece
    maxLenNew <- min( length - lSim, length(x) )
    # Sample a random segment length uniformly from available lengths
    segLen <- sample ( x = 1:maxLenNew, size = 1 )
    # Now sample a starting position, after computing the maximum starting 
    # position
    maxStart <- length - segLen + 1
    # constrain initial value to lie within predetermined interval
    ...
    if ( lSim == 0 ) 
    {
      ...
      segStart <- sample ( startIdx, size = 1 )
      if ( segStart > maxStart ) next
    } else segStart <- sample ( 1:maxStart, size = 1 )
...
callProcedureCAA <- function( obj, t )
{
  ...
  caObj$caaSurveyRel <- as.numeric( as.logical(obj$mp$assess$caaSurveyRel) )
