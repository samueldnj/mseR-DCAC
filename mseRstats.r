...
#                 and a vector with the quantiles specified by probs.
# Source:         SDN Johnson
.calcStatsClosures <- function( Dt, probs=c(0.05,0.1,0.5,0.9,0.95) )
{
  # Little function to get number of years closed
  nClosed <- function ( vec ) { length ( which (vec == 0 ) ) }
  # Compute number of closures for each replicate
  nClosures <- apply ( Dt, 1, nClosed )
  # Compute the median of the average catch values.
  probClosure <- nClosures / ncol ( Dt )
  medProbClosure <- median (probClosure)
  # Compute the quantiles of the distribution.
  qVals <- quantile( probClosure, probs=probs )
  val <- list( medProbClosure=medProbClosure, qVals=qVals )
  val
}
...
.calcStatsZones <- function( Dept, LRP, USR )
{
  ...
  {
    medCriticalP <- mean( pZone[,zoneLabels[1]] )
    medCautiousP <- mean( pZone[,zoneLabels[2]] )
    medHealthyP  <- mean( pZone[,zoneLabels[3]] )    
...
# .calcStatsFmsy (Calculate proportion of time Ft is above FMSY limit)
# Purpose:        Calculate quantiles of the proportion of time (probability)
#                 that the Ft was greater than Fmsy during time period over the 
#                  simulation replicates.
# Parameters:     Fmsy   - operating model Fmsy based on params at t==1
# Returns:        val, a scalar prob over replicates
# Source:         S.Cox, K.Holt, A.R. Kronlund
.calcStatsFmsy <- function( Ft, Fmsy )
{
  
  # Calculate number of occurences where Ft > Fmsy
  tmp <- length( Ft[Ft>Fmsy] )

  # calc prob
  prob <- tmp/length(Ft)

  val <- list( probGtFmsy=prob)
  val    
}
...
.calcPerfStats <- function()
{
  ...
  statNames   <- c( "medAvgDep","Q1AvgDep","Q2AvgDep",
                    ...
                    "probFtGtFmsy", "medProbClosure", "Q1ProbClosure",
                    "Q2ProbClosure",                   
...
#                    "medProbGteDmsy","medProbGteLimit",
  ...
  for ( i in 1:nSims )
  {
    ...
    # Extract various reference points
    ...
    if( .HISTLRP )
      ...
      LrpB0 <- mean(blob$om$Bt[ ,c(35:44), drop=FALSE ])
    LrpB0Dep   <- LrpB0 / B0
    UsrB0Dep   <- UsrB0 / B0
    TrpB0Dep   <- TrpB0 / B0
    ...
    for ( j in 1:nrow(period) )
    {
      ...
      tmp <- .calcStatsFmsy( Ft[,tdx, drop=FALSE ],Fmsy )
      result[ iRow,"probFtGtFmsy"    ] <- tmp$probGtFmsy
      ...
      #--- Closure Statistics
      tmp <- .calcStatsClosures ( Dt[, tdx, drop = FALSE ], quantVals )
      result[ iRow, "medProbClosure" ] <- tmp$medProbClosure
      result[ iRow, "Q1ProbClosure" ]  <- tmp$qVals[2]
      result[ iRow, "Q2ProbClosure" ]  <- tmp$qVals[4]
    }     # Loop over j periods.
