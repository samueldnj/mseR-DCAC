...
#                    "medProbGteDmsy","medProbGteLimit",
  ...
  # Make results dataframe for case when only those replicates that converged on
  # an answer are used to calculate stats (currently applies to production model only)
  # K.Holt, 31-Aug-09
  resultConv      <- data.frame( matrix( NA, nrow=nResults,ncol=length(colNames) ) )
  names(resultConv ) <- colNames
  ...
  for ( i in 1:nSims )
  {
    ...
    # Get the LRP, USR, TRP status values related to objectives for Bmsy and B0.
    ...
    for ( j in 1:nrow(period) )
    {
      ...
      Dt   <- blob$om$Dt[ ,c(2:ncol(blob$om$Dt)) ]
      Dept <- Bt / B0
      ...
      tmp <- .calcStatsLowCat( Dt[,tdx], quantVals )
      ...
      tmp <- .calcStatsZones( Dept[,tdx],LrpBmsyDep,UsrBmsyDep )
      ...
      tmp <- .calcStatsZones( Dept[,tdx],LrpB0Dep,UsrB0Dep )
