...
# assessModDCAC
# Purpose:       Generates Depletion-Corrected-Avg-Catch estimate of 
#                the "sustainable catch" for data-limited fisheries
# Parameters:    obj=list containing It=survey biomasses, catch, age-proportions
#                and raw parameters delta, c, M
# Returns:       DCAC sustainable catch
# Source:         S.P. Cox and SDN Johnson, based on reference 
# MacCall, A. D. 2009. Depletion-corrected average catch: a simple formula for 
# estimating sustainable yields in data-poor situations. IJMS, 66: 2267–2271.                                                      #
assessModDCAC <- function( obj ){

# obj will be assess$[var]

  # This stuff is left to ensure that graphics are unaffected
  # Compute the moving average of the survey
  # using the most recent avgPoints surveys
  It            <- obj$It
  avgPoints     <- obj$avgPoints
  dcacDist      <- obj$dcacDist
  trend         <- obj$trend
  state         <- obj$state
  dcacPosGrad   <- obj$assess$dcacPosGrad
  dcacNegGrad   <- obj$assess$dcacNegGrad
  dcacCons      <- obj$assess$dcacCons
  quantUB       <- obj$quantUB
  MSYest        <- obj$MSYest
  dcacQuant     <- obj$assess$dcacQuant
  dcacState     <- obj$assess$dcacState
  ...
  movinAvg <- mean( tmp[tVec] )

  if ( ! dcacState ) state <- 1

  # if using quantile based rule:
  if ( dcacQuant )
  {
    # Set starting quantile
    quant0 <- quantUB * dcacCons

    # Set quantile based on trend and state
    if ( state > 1 )
    {
      if ( trend >= 0 ) quant <- min ( quantUB, quant0 + trend * dcacPosGrad )
      if ( trend < 0 )  quant <- max ( 0, quant0 + trend * dcacNegGrad/state )
    }
    if ( state == 1 )
    {
      if ( trend >= 0 ) quant <- min ( quantUB, quant0 + trend * dcacPosGrad )
      if ( trend < 0 ) quant <-  max ( 0, quant0 + trend * dcacNegGrad )
    }
    if ( state < 1 )
    {
      if ( trend >= 0 ) quant <- min ( quantUB, state * quant0 + trend * dcacPosGrad )
      if ( trend < 0 ) quant <-  max ( 0, state * quant0 + trend * dcacNegGrad/state ) 
    }
  } else # use an absolute based rule
  {
    # Set initial catch based on a conservative factor of MSY
    catch0 <- MSYest * dcacCons

    # Now adjust according to state and trend:
    if ( state > 1 )
    {
      if ( trend >= 0 ) catch <- min ( MSYest, catch0 + trend * dcacPosGrad )
      if ( trend < 0 )  catch <- max ( 0, catch0 + trend * dcacNegGrad/state )
    }
    if ( state == 1 )
    {
      if ( trend >= 0 ) catch <- min ( MSYest, catch0 + trend * dcacPosGrad )
      if ( trend < 0 ) catch <-  max ( 0, catch0 + trend * dcacNegGrad )
    }
    if ( state < 1 )
    {
      if ( trend >= 0 ) catch <- min ( state * MSYest, state * catch0 + trend * dcacPosGrad )
      if ( trend < 0 ) catch <-  max ( 0, state * catch0 + trend * dcacNegGrad/state ) 
    }
  }


  assessment <- list()
  assessment$exploitBt <- movinAvg
  assessment$spawnBt   <- movinAvg
  if ( dcacQuant ) assessment$dcac      <- quantile(dcacDist, probs=quant )
  else assessment$dcac  <- catch
...
callProcedureDCAC <- function( obj, t )
{
  # Extract operating model and management procedure objects
  om <- obj$om
  mp <- obj$mp

  # Recover the initial time step for the management procedure
  tMP <- obj$opMod$tMP

  # Extract catch history.
  Dt <- om$Dt[1:(t-1)]

  # If this is the first year of the management procedure, then
  # find the DCAC distribution which is sampled from now on.
  if ( t == tMP )
  {
    # For testing the DCAC procedure
    if ( as.logical(mp$assess$dcacTest) )
    {
      # Recover multiplier and M value
      M <- mean (obj$om$Mt) * mp$assess$dcacMortMult
      Delta <- ( obj$opMod$B0 - obj$om$Bt[tMP-1] ) / 
                 obj$opMod$B0 * mp$assess$dcacDeltaMult
    } else { # Take the "guesses" from the simCtlFile
      M         <- mp$assess$dcacM  
      Delta     <- mp$assess$dcacDelta
    }

    # Sumcatch
    if ( is.null(mp$assess$dcacWindow ) ) sumCatch <- sum ( Dt [ 1:(tMP-1) ] )
    else sumCatch <- sum ( Dt [ (tMP - mp$assess$dcacWindow):(tMP-1) ] )

    # Recover remaining parameters from simCtlFile
    sdLogM    <- mp$assess$dcacCVLogM   
    sdDelta   <- mp$assess$dcacSDDelta
    C         <- mp$assess$dcacC
    sdC       <- mp$assess$dcacSDC
    nSim      <- mp$assess$dcacSim

    # Generate distributions of dcac parameters
    ranM     <- M*exp( sdLogM*rnorm( n=nSim,mean=0,sd=1 ) - sdLogM*sdLogM/2. )
    ranDelta <- rnorm ( n = nSim, mean = Delta, sd = sdDelta)
    ranDelta[ranDelta <= 0] <- 0.01
    ranC     <- rnorm( n=nSim, mean=C, sd=sdC )
    ranC [ ranC <= 0 ] <- 0.01

    # Main DCAC calculation
    winFallRatio <- (1 - ranDelta)/(0.4*ranC*ranM)
    if ( is.null(mp$assess$dcacWindow ) ) 
    {
      dcacDist  <<- sumCatch/((tMP - 1) + winFallRatio)
    }
    else dcacDist  <<- sumCatch/((mp$assess$dcacWindow) + winFallRatio)

    # Now solve for MSY quantile (most probable quantity) and set as the
    # upper bound on the trend-based sampling.
    h <- hist ( dcacDist, breaks = 200, plot = FALSE )
    MSYest <<- h$mids[h$counts == max(h$counts)]
    cdf <- ecdf ( dcacDist )
    quantUB <<- cdf ( MSYest )
  }
  ...
  ItDCAC <- It [ c ( (tMP-1):(t-1) ) ]
  ItDCAC <- ItDCAC[ !is.na(ItDCAC) ]
  logItDCAC <- log( ItDCAC )

  # Pull out trend based DCAC timeframe and probability thresholds
  nPoints <- mp$assess$dcacTimeFrame
  posThresh <- mp$assess$dcacPosTrendProb
  negThresh <- mp$assess$dcacNegTrendProb

  # compute length of ItDCAC
  l <- length ( ItDCAC )

  # Fit a linear model to the It series to get a trend
  if ( l <= 2 ) # If time series is too short (df < 1) then set trend = 0
  {
    trend <- 0
  } else # if there are >= 3 points, estimate a trend
  {
    # preprocess data
    nPoints <- min ( length (logItDCAC), nPoints ) 
    tSeries <- 1:( nPoints )
    logItDCAC <- logItDCAC [ (l - nPoints + 1):l]
    # Fit a linear model
    trendModel <- lm ( logItDCAC ~ tSeries )
    # Pull out coefficients and t-score
    trend <- coef ( trendModel ) [ 2 ]
    tscore <- summary ( trendModel ) [[ 4 ]] [ 2, 3 ]
    # Run NHST on trend detection
    if ( trend > 0 ) # positive trend
    {
      p <- pt ( tscore, df = nPoints - 2, lower.tail = FALSE )
      if ( p > posThresh ) trend <- 0 
    } else if ( trend < 0 ) { # negative trend
      p <- pt ( tscore, df = nPoints - 2 )
      if ( p > negThresh ) trend <- 0
    }
  }

  # compute state
  state <- ItDCAC [ l ] / ItDCAC [ 1 ]
 
  # Assessment frequency information - not implemented yet
  
  tmp             <- list()                 # Temporary list
  tmp$trend       <- trend                  # Stock index trend over given time
  tmp$state       <- state                  # stock state compared to tMP
  tmp$It          <- It                     # Stock Index      
  tmp$Dt          <- Dt                     # Catch history
  tmp$dcacDist    <- dcacDist               # Distribution of DCAC values
  tmp$quantUB     <- quantUB                # upper bound on dcac quantiles (MSY)
  tmp$MSYest      <- MSYest                 # estimate of MSY
  tmp$avgPoints   <- mp$assess$avgPoints    # Number of index points to average
  tmp$assess      <- mp$assess              # pass assessment parameters
  stockAssessment <- assessModDCAC( tmp )   # Call DCAC method
  
  stockAssessment
}     # END function callProcedureDCAC
...
.updatePop <- function( obj, t )
{
  ...
  if ( methodId == .USER )
  {
    stockAssessment <- callProcedureDCAC ( obj, t )
  ...
  else
  {
    ...
    else
    {
      ...
      # browser() # For some reason this draws from an NA row, not the first row
      if ( obj$mp$hcr$statusBase == "statusBaseB0" )
  ...
  if ( (obj$mp$assess$methodId==.USER) )
  {
    hcrList$catchLimit <- stockAssessment$dcac
  ...
  if ( obj$mp$assess$methodId==.USER ) targetHarv <- calcHCRdcac ( hcrList )
  else if ( (obj$mp$assess$methodId==.MOVAVG) | (obj$mp$assess$methodId==.KALMAN) )
  {
...
calcHCRdcac <- function( hcrList )
{
  ...
  # Get catch limit set by dcacDist sample
  result <- hcrList$catchLimit
  ...
  # # If using DCAC method, maximise the catch at the DCAC estimate
  # if ( hcrList$methodId == .USER )
  # {
  #   result <- min ( result, hcrList$catchLimit )
  # }
